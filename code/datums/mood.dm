...
/datum/mood/proc/print_mood(mob/user)
	var/msg = "[span_info("<EM>Мое психическое состояние:</EM>")]\n"
	msg += span_notice("Мой рассудок: ") //Long term
	//ORIGINAL
	/*
	switch(sanity)
	...
	*/
	...
	if(!HAS_TRAIT(user, TRAIT_MOOD_NOEXAMINE))
		switch(sanity)
			if(SANITY_GREAT to INFINITY)
				msg += "[span_nicegreen("Мой разум, мой храм!")]\n"
			if(SANITY_NEUTRAL to SANITY_GREAT)
				msg += "[span_nicegreen("В принципе я чувствую себя хорошо!")]\n"
			if(SANITY_DISTURBED to SANITY_NEUTRAL)
				msg += "[span_nicegreen("Ну, я чувтсвую себя нормально.")]\n"
			if(SANITY_UNSTABLE to SANITY_DISTURBED)
				msg += "[span_warning("Я чувствую себя не в свой тарелке...")]\n"
			if(SANITY_CRAZY to SANITY_UNSTABLE)
				msg += "[span_boldwarning("Я схожу с ума!!!")]\n"
			if(SANITY_INSANE to SANITY_CRAZY)
				msg += "[span_boldwarning("АХАХАХАХАХАХАХАХАХА!!")]\n"
	else
	...
	*/
	...
	if(!HAS_TRAIT(user, TRAIT_MOOD_NOEXAMINE))
		switch(mood_level)
			if(MOOD_LEVEL_SAD4)
				msg += "[span_boldwarning("Я думаю, я хочу умереть!")]\n"
			if(MOOD_LEVEL_SAD3)
				msg += "[span_boldwarning("Я чувствую себя ужасно...")]\n"
			if(MOOD_LEVEL_SAD2)
				msg += "[span_boldwarning("Я чувствую сильную грусть.")]\n"
			if(MOOD_LEVEL_SAD1)
				msg += "[span_warning("Мне немного грустно.")]\n"
			if(MOOD_LEVEL_NEUTRAL)
				msg += "[span_grey("Я в норме")]\n"
			if(MOOD_LEVEL_HAPPY1)
				msg += "[span_nicegreen("Я чувствую себя неплохо.")]\n"
			if(MOOD_LEVEL_HAPPY2)
				msg += "[span_boldnicegreen("Я чувствую себя довольно хорошо.")]\n"
			if(MOOD_LEVEL_HAPPY3)
				msg += "[span_boldnicegreen("Я чувствую себя потрясающе!")]\n"
			if(MOOD_LEVEL_HAPPY4)
				msg += "[span_boldnicegreen("Я люблю жизнь!")]\n"
	else
		msg += "[span_notice("Без понятия.")]\n"

	msg += "[span_notice("Реакции:")]\n"//All moodlets
