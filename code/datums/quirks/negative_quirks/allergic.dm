...
/datum/quirk/item_quirk/allergic/add(client/client_source)
	...
	name = "Острая аллергия на [allergy_string]"
	medical_record_text = "Иммунная система пациента остро реагирует на [allergy_string]"
...
/datum/quirk/item_quirk/allergic/post_add()
	...
	to_chat(quirk_holder, span_boldnotice("У вас аллергия на [allergy_string], не употребляйте эти продукты!"))
