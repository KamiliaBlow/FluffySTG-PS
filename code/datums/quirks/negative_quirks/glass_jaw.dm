/datum/quirk/glass_jaw
	name = "Glass Jaw"
	desc = "У вас очень хрупкая челюсть. Любой достаточно сильный удар по голове может вырубить вас."
	icon = FA_ICON_HAND_FIST
	value = -4
	gain_text = span_danger("У вас болит челюсть.")
	lose_text = span_notice("Ваша челюсть снова чувствует себя нормально.")
	medical_record_text = "Пациента очень легко вырубить. Не подпускайте его к боксерскому рингу."
	hardcore_value = 4
	mail_goodies = list(
	...
	if(prob(CEILING(damage * (sharpness & (SHARP_EDGED|SHARP_POINTY) ? 0.65 : 1), 1)))
		...
		if(!source.IsUnconscious())
			source.visible_message(
				span_warning("[source] в нокауте!"),
				span_userdanger("Вы нокаутированы!"),
