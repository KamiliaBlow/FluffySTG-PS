/datum/quirk/item_quirk/scarred_eye
	...
	value = -2
	gain_text = span_danger("Даже после всего прошедшего времени, ваш глаз все еще немного щиплет...")
	lose_text = span_notice("Ваше периферическое зрение увеличивается примерно на тридцать процентов.")
	medical_record_text = "У пациента имеется выраженное рубцевание одного глаза, приводящее к частичной потере зрения."
	hardcore_value = 2
