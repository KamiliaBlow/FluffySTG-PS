/datum/quirk/item_quirk/addict
	name = "Addict"
	desc = "Вы зависимы от чего-то, чего не существует. Страдайте."
	gain_text = span_danger("Вы внезапно почувствовали тягу к... чему-то? Вы не уверены, к чему именно.")
	medical_record_text = "У пациента есть история с ЧЕМ-ТО, но он отказывается нам сказать, что это такое."
...
/datum/quirk/item_quirk/addict/process(seconds_per_tick)
	...
	if(deleted || missing_addiction)
		if(deleted)
			reagent_instance = new reagent_type()
		to_chat(quirk_holder, span_danger("Вы думали, что покончили с этим, но чувствуете, что снова возвращаетесь к старым привычкам..."))
		for(var/addiction in reagent_instance.addiction_types)
			human_holder.last_mind?.add_addiction_points(addiction, 1000) ///Max that shit out

/datum/quirk/item_quirk/addict/junkie
	name = "Junkie"
	desc = "Вы не можете насытиться тяжелыми наркотиками."
	icon = FA_ICON_PILLS
	value = -6
	gain_text = span_danger("Вы внезапно чувствуете тягу к наркотикам.")
	medical_record_text = "У пациента в анамнезе употребление тяжелых наркотиков."
	hardcore_value = 4
	mail_goodies = list(/obj/effect/spawner/random/contraband/narcotics)
	drug_flavour_text = "Лучше надейтесь, что у вас не закончится..."
...
/datum/quirk/item_quirk/addict/smoker
	name = "Smoker"
	desc = "Иногда просто очень хочется покурить. Наверное, это не очень хорошо для легких."
	icon = FA_ICON_SMOKING
	value = -4
	gain_text = span_danger("Сейчас бы действительно хотелось покурить.")
	...
	hardcore_value = 1
	drug_flavour_text = "Убедитесь, что вы покупаете свою любимую марку, когда она заканчивается."
	mail_goodies = list(
...
/datum/quirk/item_quirk/addict/alcoholic
	...
	lose_text = span_notice("Алкоголь больше не кажется таким привлекательным.")
	medical_record_text = "Пациент страдает алкоголизмом."
	...
	hardcore_value = 1
	drug_flavour_text = "Убедитесь, что у вас есть ваш любимый напиток, когда он закончится."
	mail_goodies = list(
