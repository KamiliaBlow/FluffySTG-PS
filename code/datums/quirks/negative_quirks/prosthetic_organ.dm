/datum/quirk/prosthetic_organ
	...
	icon = FA_ICON_LUNGS
	value = -3
	medical_record_text = "При физическом осмотре у пациента был обнаружен дешевый протез органа. \
...
/datum/quirk/prosthetic_organ/add_unique(client/client_source)
	...
	switch(organ_slot)
		...
		if(ORGAN_SLOT_STOMACH)
			...
			slot_string = "stomach"
	medical_record_text = "При физическом осмотре у пациента был обнаружен дешевый протез [slot_string]. \
...
/datum/quirk/prosthetic_organ/post_add()
	to_chat(quirk_holder, span_bolddanger("Ваша [slot_string] был заменен дешевым протезом органа. Оно слабо и крайне нестабильно. \
	Кроме того, любой ЭМИ разряд приведет к полному прекращению его работы."))
