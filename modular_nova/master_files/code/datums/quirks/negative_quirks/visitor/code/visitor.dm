/datum/quirk/visitor
	name = "Удостоверение посетителя"
	desc = "Как ассистент, вы получаете удостоверение посетителя с ограниченной предысторией. Ваши записи будут скудными и потребуют заполнения в течение смены."
	icon = FA_ICON_PERSON_CIRCLE_QUESTION
	value = 0 // FLUFFY FRONTIER CHANGE: was -2
	medical_record_text = "Пациент является гостем на станции, ему выдано удостоверение посетителя."
	gain_text = span_notice("Как гость на станции, вы получили специальное удостоверение посетителя!")
	lose_text = span_danger("Ваши права на посещение отозваны...")
